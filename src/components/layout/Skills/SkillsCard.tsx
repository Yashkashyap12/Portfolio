        ...
        <div className="py-16 px-6">
            ...
            >
                {skills.map((skill, idx) => (
                    <div
                        key={idx}
                        onClick={() => handleCardClick(idx)} // tap-to-flip
                        className="group relative w-full h-80 cursor-pointer"
                    >
                        <div
                            className={`relative w-full h-full transition-transform duration-700 transform-style-preserve-3d ${flippedIndex === idx ? 'rotate-y-180' : 'group-hover:rotate-y-180'
                                }`}
                        >
                            {/* Front */}
                            <div className="absolute inset-0 bg-purple-500/10 border border-purple-500 backdrop-blur-md rounded-2xl shadow-[0_8px_30px_rgb(0,0,0,0.12)] flex items-center justify-center backface-hidden">
                                <Image
                                    src={skill.img}
                                    alt={skill.name}
                                    width={180}
                            ...
                            {/* Back */}
                            <div className="absolute inset-0 bg-purple-500/10 border border-purple-500 backdrop-blur-md text-black rounded-2xl shadow-[0_8px_30px_rgb(0,0,0,0.2)] flex flex-col items-center justify-center rotate-y-180 backface-hidden px-4 text-center">
                                <span className="text-sm drop-shadow-md text-[#05045287]">{skill.description}</span>
